...
class TextField(pygame.sprite.Sprite):
    ...
    def __init__(self, font, initial_text="", label_text="", position=(0, 0), max_length=20, padding=5):
        ...
        self.label_surface = self.font.render(self.label_text, True, self.text_color) if label_text else None
        
        self.image = pygame.Surface((self.text_surface.get_width() + 2 * self.padding, self.text_surface.get_height() + 2 * self.padding))
    ...
    def update(self):
        text_surface = self.font.render(self.text if self.text else " ", True, self.text_color)
        label_surface = self.font.render(self.label_text, True, self.text_color) if self.label_text else None

        text_width = text_surface.get_width() + 2 * self.padding
        label_width = label_surface.get_width() + 2 * self.padding if label_surface else 0
        ...
        pygame.draw.rect(self.image, self.border_color, self.input_rect, self.border_width)
