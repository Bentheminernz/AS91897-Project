...
class Checkbox(pygame.sprite.Sprite):
    ...
    def update_image(self):
        ...
        # Draw the checkbox
        pygame.draw.rect(self.image, self.border_color, (0, (total_height - self.size) // 2, self.size, self.size), 2)
        
        # If checked, fill with the check color
        if self.is_checked:
            pygame.draw.rect(self.image, self.check_color, 
                            (3, (total_height - self.size) // 2 + 3, self.size - 6, self.size - 6))
        
        # Draw the text
        self.image.blit(text_surface, (self.size + 10, (total_height - text_surface.get_height()) // 2))
